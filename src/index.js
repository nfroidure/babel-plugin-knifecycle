        ...
        path.traverse({
          ImportSpecifier(path) {
            ...
            if ('autoHandler' === importedNode.node.name) {
              ...
              path.scope.rename(
                localNode.node.name,
                path.scope.generateUidIdentifier('handler').name,
              );
              ...
              binding.referencePaths.forEach(path => {
                ...
                const autoHandlerArgumentPath = path.parentPath.get(
                  'arguments.0',
                );
                let name = '';
                let injections = [];
                ...
                if (autoHandlerArgumentPath.isIdentifier()) {
                  ...
                  if (!handlerArgumentPath.isObjectPattern()) {
                    throw autoHandlerArgumentPath.buildCodeFrameError(
                      'Expect the function passed in autoHandler to have an object pattern as first argument.',
                    );
                  }
                  ...
                  const handlerArgumentPath = autoHandlerArgumentPath.get(
                    'params.0',
                  );
